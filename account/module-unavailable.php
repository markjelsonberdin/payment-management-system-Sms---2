<?php
/**
 * SMS 2 – Module unavailable (per-module maintenance)
 * Calm crest + soft “under maintenance” cue. Sign out only.
 */
...
$moduleKey = strtolower(trim((string) ($_GET['module'] ?? '')));
...
if (!smsIsModuleInMaintenance($moduleKey)) {
    $path = $moduleKey === 'student_portal'
        ? BASE_URL . '/modules/student-portal/pages/my-profile.php'
        : BASE_URL . '/modules/' . rawurlencode($moduleKey) . '/index.php';
    header('Location: ' . $path);
    exit;
}

$label = function_exists('smsModuleLabel') ? smsModuleLabel($moduleKey) : $moduleKey;
$message = smsModuleMaintenanceMessage($moduleKey);
